...
create temporary table qa_fixture_classes (
...
insert into qa_fixture_classes
select
...
from (values
  ...
  (md5('qa-class:11c-computing-archive')::uuid, '11c-computing-archive', '11C Computing (2023/24)', 11::smallint, '50000000-0000-4000-8000-000000000001'::uuid, 'archived', 25, 11),
  (md5('qa-class:13d-it-archive')::uuid, '13d-it-archive', '13D IT (2023/24)', 13::smallint, '50000000-0000-4000-8000-000000000002'::uuid, 'archived', 25, 12)
...
create temporary table qa_fixture_students (
...
insert into qa_fixture_students
select
...
using qa_fixture_students fixture
...
create temporary table qa_fixture_learners (student_id uuid primary key, profile_id uuid not null, class_id uuid not null, class_order integer not null, row_number integer not null, account_status text not null);

insert into qa_fixture_learners
...
insert into qa_fixture_learners
select student_id, profile_id, class_id, class_order, row_number, account_status from qa_fixture_students;

create temporary table qa_fixture_assignments (
  id uuid primary key,
...
insert into qa_fixture_assignments
...
where assignment_id in (select id from qa_fixture_assignments);

insert into public.assignment_recipients (assignment_id, student_id)
select assignment.id, learner.student_id
from qa_fixture_assignments assignment
join qa_fixture_learners learner on learner.class_id = assignment.class_id
...
set
  ...
  updated_at = now()
from qa_fixture_assignments assignment
join qa_fixture_learners learner
...
from qa_fixture_learners learner
cross join lateral generate_series(1, 6) practice_number
join qa_fixture_classes class on class.class_id = learner.class_id
cross join lateral (
...
where attempt.suspicious_event_count > 0
  and attempt.assignment_id in (select id from qa_fixture_assignments)
on conflict (id) do update set
...
from public.test_attempts attempt
join qa_fixture_learners learner on learner.student_id = attempt.student_id and learner.class_id = attempt.class_id_at_attempt
where attempt.points_awarded > 0
  and (
    attempt.assignment_id in (select id from qa_fixture_assignments)
...
where student_id in (select student_id from qa_fixture_learners);
...
from qa_fixture_learners learner where learner.row_number % 3 = 0
...
drop table if exists qa_fixture_assignments;
drop table if exists qa_fixture_learners;
drop table if exists qa_fixture_students;
drop table if exists qa_fixture_classes;
